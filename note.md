...
基于策略的方法首先需要将策略参数化。假设目标策略是一个随机性策略，并且处处可微，其中是对应的参数。我们可以用一个线性模型或者神经网络模型来为这样一个策略函数建模，输入某个状态，然后输出一个动作的概率分布。我们的目标是要寻找一个最优策略并最大化这个策略在环境中的期望回报。将策略学习的目标函数定义为
$$
J(\theta) = \mathbb{E}_{s_0}[V^{\pi_{\theta}}(s_0)]
$$
其中，$s_0$表示初始状态。现在有了目标函数，我们将目标函数对策略求导，得到导数后，就可以用梯度上升方法来最大化这个目标函数，从而得到最优策略。

我第 3 章讲解过策略$\pi$下的状态访问分布，在此用表示。然后对目标函数求梯度，可以得到如下式子:

![image-20241029084917037](image/note/image-20241029084917037.png)
...
<img src="image/note/640.e6d241d0.png" alt="img" style="zoom: 50%;" />
...
![image-20241029085044496](image/note/image-20241029085044496.png)

REINFORCE的具体算法步骤如下：

![image-20241029085108557](image/note/image-20241029085108557.png)

另外，可以加baseline提高训练效果。

为了在REINFORCE算法中加入baseline，可以使用状态值函数 $ V(s) $ 作为baseline。引入baseline可以减少回报的方差，从而加速收敛。具体实现上，我们可以增加一个网络来预测每个状态的价值 $ V(s) $，并在更新策略时用 $ R - V(s) $ 代替原始的回报 $ R $，从而减少方差。

### A2C

<img src="image/note/640.5dee5ab0.jpg" alt="img" style="zoom:80%;" />

![image-20241029100420328](image/note/image-20241029100420328.png)
