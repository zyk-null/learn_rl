# Dueling DQN适合复杂一点的环境，在carpole这个简单的环境中，效果不如DDQN好
import gym
import random
import numpy as np
import torch
...
class Config:
    def __init__(self):
        self.env_name = 'CarPole-v0'
        self.algo_name = 'DuelingDQN'
        self.train_eps = 500
        self.test_eps = 5
...
class VDNet(nn.Module):
    def __init__(self, n_states, n_actions, hidden_dim=128):
        ...
        self.advantage_stream = nn.Sequential(
            nn.Linear(hidden_dim, 64),
            nn.ReLU(),
            nn.Linear(64, n_actions)  # 输出 A(s, a)
        )
    ...
    def forward(self, x):
        ...
        # 合并 V(s) 和 A(s, a) 以计算 Q(s, a)
        q_values = values + (advantages - advantages.mean(1).view(-1, 1))
        return q_values
...
def env_agent_config(cfg):
    ...
    policy_net = torch.jit.script(VDNet(n_states, n_actions, cfg.hidden_dim).to(cfg.device)) 
    target_net = torch.jit.script(VDNet(n_states, n_actions, cfg.hidden_dim).to(cfg.device)) 
    memory = ReplayBuffer(cfg.memory_capacity)
    agent = DuelingDQN(policy_net, target_net, memory, cfg)
    return env, agent
